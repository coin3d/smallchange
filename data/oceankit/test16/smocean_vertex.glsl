...
// fake function used to test
void calc_once(out vec4 p,
               out vec3 n,
               out vec3 t)
...
void main(void)
{
  ...
  // calc four times to simulate 16 waves
  calc_once(p, n0, t0);
  calc_once(p, n1, t1);
  calc_once(p, n2, t2);
  calc_once(p, n3, t3);

  vec3 n = normalize(n0 + n1 + n2 + n3);
  vec3 t = normalize(t0 + t1 + t2 + t3);
