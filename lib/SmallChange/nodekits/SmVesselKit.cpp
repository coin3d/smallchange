...
SmVesselKit::GLRender(SoGLRenderAction * action)
{
  ...
  if (ok && ok->isOfType(SmOceanKit::getClassTypeId())) {
    ...
    elevation = (e1+e2)/2.0;
    this->pitch.enableNotify(FALSE);  // Avoid triggering redraw
    this->pitch.setValue(pitch);
    this->pitch.enableNotify(TRUE);
    this->roll.enableNotify(FALSE);  // Avoid triggering redraw
  ...
  this->position.enableNotify(FALSE);  // Avoid triggering redraw
